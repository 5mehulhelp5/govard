...
```bash
govard profile
govard profile --framework laravel --framework-version 11
govard profile --json
govard profile apply
govard profile apply --framework magento2 --framework-version 2.4.7
```

## Behavior

- `govard profile` is read-only.
- `govard profile apply` writes the selected profile into `govard.yml`.
