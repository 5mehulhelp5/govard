...
1. Run `govard deps`.
2. Run `govard init`.
3. Inspect profile with `govard profile`.
4. Apply profile if needed: `govard profile apply`.
5. Start runtime: `govard up`.
6. Use remote operations (`remote`, `sync`, `db`) during development.

## PM Journey

1. Install Govard from project onboarding docs.
2. Run `govard deps` and follow dependency guidance.
3. Run `govard up`.
