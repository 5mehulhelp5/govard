...
- Govard exports `GOVARD_HOOK_EVENT`, `GOVARD_PROJECT_NAME`, `GOVARD_DOMAIN`, and `GOVARD_FRAMEWORK` while executing hooks.
...
Static assets like `varnish/default.vcl` are stored in `blueprints/[framework]/` and mapped to containers during `govard env up`.
...
2. Add optional support files in `blueprints/[framework]/`
