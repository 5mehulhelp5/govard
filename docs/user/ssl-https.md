...
1. **Locate the CA**: The certificate is exported to `~/.govard/ssl/root.crt` after running `govard trust` on Linux.
...
Govard uses Caddy's internal PKI to manage local certificates. The root certificate is generated inside the `proxy-caddy-1` container. On Linux, `govard trust` extracts it to `~/.govard/ssl/root.crt`.

### How It Works

1. **Certificate Generation**: When you run `govard up`, Caddy generates a certificate for your `.test` domain.
...
**Solutions**:
1. Ensure proxy is running: `govard proxy start`
2. Run `govard trust` again
3. On macOS, re-export `/tmp/govard-ca.crt` then rerun `govard trust`
4. Manually import the certificate in your browser (see step 2 above)
5. On Linux, check that `~/.govard/ssl/root.crt` exists after `govard trust`

**Issue**: Certificate not generated

**Solution**:
1. Ensure `govard up` completed successfully
2. Check Caddy proxy is running: `docker ps | grep caddy`
3. Check logs: `govard logs`
