export const normalizeSettingsPayload = (settings = {}) => ({
  ...
  preferredBrowser: settings.preferredBrowser || settings.PreferredBrowser || "",
})
...
export const createSettingsController = ({ bridge, refs, onStatus, onToast }) => {
  ...
  const load = async () => {
    try {
      ...
      if (refs.preferredBrowser) refs.preferredBrowser.value = settings.preferredBrowser
  ...
  const save = async () => {
    ...
    try {
      const message = await bridge.updateSettings(theme, proxyTarget, preferredBrowser)
      applyTheme(theme)
      onStatus(message)
