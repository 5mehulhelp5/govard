...
export const createOnboardingController = ({ bridge, refs, onStatus, onToast, onProjectAdded }) => {
  ...
  const addProject = async () => {
    ...
    const message = String((await bridge.onboardProject(projectPath, recipe)) || "")
    const isError = message.toLowerCase().includes("failed")
    onStatus(message || "Status: project onboarding finished")
    onToast(message || "Project onboarding finished", isError ? "error" : "success")
