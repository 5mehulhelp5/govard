...
`
		_ = os.MkdirAll(filepath.Dir(appConfigPath), DefaultDirPerm)
		if err := os.WriteFile(appConfigPath, []byte(content), SecretFilePerm); err != nil {
	...
	if _, err := os.Stat(appConfigPath); err == nil {
		...
		if err == nil {
			...
			if !strings.Contains(updated, "'host' => 'db'") {
				...
				_ = os.WriteFile(appConfigPath, []byte(updated), SecretFilePerm)
			}
		}
	}
...
func (c *CakePHPBootstrap) PostClone(projectDir string) error {
	...
	if _, err := os.Stat(appConfigPath); os.IsNotExist(err) {
		...
		if _, err := os.Stat(appDefaultPath); err == nil {
			...
			_ = os.WriteFile(appConfigPath, []byte(content), SecretFilePerm)
		}
	}
